...
public interface SysPermMapper {
    ...
    public Long     selectCount(SysPerm sysPerm);
    public List<SysPerm> selectPage(SysPerm sysPerm);
    public List<SysPerm> select(SysPerm sysPerm);
    public SysPerm       selectOne(SysPerm sysPerm);
    public List<SysPerm> selectAll();
    public SysPerm       selectByPk(Integer id);


    /** 主键或唯一索引冲突时，报错 */
    public int insert(SysPerm sysPerm);
    /** 主键或唯一索引冲突时，不报错，就丢弃此记录，但mysql有warning日志 */
    public int insertOrIgnore(SysPerm sysPerm);
    /** 主键或唯一索引冲突时，执行更新操作 */
    public int insertOrUpdate(SysPerm sysPerm);
    ...
    */
    ...
    public int insertMulti(List<SysPerm> list);
    /** 1个insert多个values，主键或唯一索引冲突时，不报错，丢弃冲突的记录，其它记录正常入库，但mysql有warning日志 */
    public int insertMultiOrIgnore(List<SysPerm> list);
    ...
    public int insertMultiOrUpdateWithNull(List<SysPerm> list);
    /** 1个insert多个values，主键或唯一索引冲突时，执行更新操作，如果字段值为空，最终会被设置为null，更新时，丢弃excludeProperties里的字段 */
    public int insertMultiOrUpdateWithNullExcludeUpdateProperty(List<SysPerm> list, Collection<String> excludeProperties);
    ...
    */
    ...
    public int multiInsert(List<SysPerm> list);
    public int multiInsertOrIgnore(List<SysPerm> list);
    public int multiInsertOrUpdate(List<SysPerm> list);
    public int multiInsertOrUpdateWithNull(List<SysPerm> list);


    /** 注意，pk必须设置值，否则报错，其它字段值为null，则丢弃此字段的更新 */
    public int update(SysPerm sysPerm);
    /** 注意，pk必须设置值，否则报错，其它字段值为null，则mysql实际也会将该字段设置为null */
    public int updateWithNull(SysPerm sysPerm);
    /** 注意，pk必须设置值，否则报错，set column=column+? where id=? */
    public int updateWithInc(SysPerm sysPerm);
    /** 注意，pk必须设置值，否则报错，乐观锁，表必须有version字段, 否则报错 */
    public int updateWithVersion(SysPerm sysPerm);

    /** 注意，自定义更新条件，字段值为null，则丢弃此字段的更新 */
    public int updateWhere(SysPerm sysPerm);
    /** 注意，自定义更新条件，字段值为null，则mysql实际也会将该字段设置为null */
    public int updateWhereWithNull(SysPerm sysPerm);
    ...
    */
    ...
    public int multiUpdate(List<SysPerm> list);
    /** 注意，自定义更新条件，字段值为null，则丢弃此字段的更新，mysql连接必须开启 allowMultiQueries=true */
    public int multiUpdateWhere(List<SysPerm> list);
    /** 注意，自定义更新条件，字段值为null，则mysql实际也会将该字段设置为null ，mysql连接必须开启 allowMultiQueries=true */
    public int multiUpdateWhereWithNull(SysPerm sysPerm);


    public int deleteByPk(Integer id);
    public int delete(SysPerm sysPerm);
