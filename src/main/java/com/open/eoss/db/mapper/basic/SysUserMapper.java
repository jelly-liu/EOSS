...
public interface SysUserMapper {
    ...
    public Long     selectCount(SysUser sysUser);
    ...
    public int insertOrIgnore(SysUser sysUser);
    /** 主键或唯一索引冲突时，执行更新操作 */
    public int insertOrUpdate(SysUser sysUser);
    ...
    */
    ...
    public int insertMulti(List<SysUser> list);
    /** 1个insert多个values，主键或唯一索引冲突时，不报错，丢弃冲突的记录，其它记录正常入库，但mysql有warning日志 */
    public int insertMultiOrIgnore(List<SysUser> list);
    ...
    public int insertMultiOrUpdateWithNull(List<SysUser> list);
    /** 1个insert多个values，主键或唯一索引冲突时，执行更新操作，如果字段值为空，最终会被设置为null，更新时，丢弃excludeProperties里的字段 */
    public int insertMultiOrUpdateWithNullExcludeUpdateProperty(List<SysUser> list, Collection<String> excludeProperties);
    ...
    */
    ...
    public int multiInsert(List<SysUser> list);
    public int multiInsertOrIgnore(List<SysUser> list);
    public int multiInsertOrUpdate(List<SysUser> list);
    public int multiInsertOrUpdateWithNull(List<SysUser> list);
    ...
    public int updateWithInc(SysUser sysUser);
    /** 注意，pk必须设置值，否则报错，乐观锁，表必须有version字段, 否则报错 */
    public int updateWithVersion(SysUser sysUser);

    /** 注意，自定义更新条件，字段值为null，则丢弃此字段的更新 */
    public int updateWhere(SysUser sysUser);
    /** 注意，自定义更新条件，字段值为null，则mysql实际也会将该字段设置为null */
    public int updateWhereWithNull(SysUser sysUser);
