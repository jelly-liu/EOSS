...
public interface SysUserPermMapper {
    ...
    public Long     selectCount(SysUserPerm sysUserPerm);
    public List<SysUserPerm> selectPage(SysUserPerm sysUserPerm);
    public List<SysUserPerm> select(SysUserPerm sysUserPerm);
    public SysUserPerm       selectOne(SysUserPerm sysUserPerm);
    public List<SysUserPerm> selectAll();
    public SysUserPerm       selectByPk(Integer userId, Integer permId);


    /** 主键或唯一索引冲突时，报错 */
    public int insert(SysUserPerm sysUserPerm);
    /** 主键或唯一索引冲突时，不报错，就丢弃此记录，但mysql有warning日志 */
    public int insertOrIgnore(SysUserPerm sysUserPerm);
    /** 主键或唯一索引冲突时，执行更新操作 */
    public int insertOrUpdate(SysUserPerm sysUserPerm);
    ...
    */
    ...
    public int insertMulti(List<SysUserPerm> list);
    /** 1个insert多个values，主键或唯一索引冲突时，不报错，丢弃冲突的记录，其它记录正常入库，但mysql有warning日志 */
    public int insertMultiOrIgnore(List<SysUserPerm> list);
    ...
    public int insertMultiOrUpdateWithNull(List<SysUserPerm> list);
    /** 1个insert多个values，主键或唯一索引冲突时，执行更新操作，如果字段值为空，最终会被设置为null，更新时，丢弃excludeProperties里的字段 */
    public int insertMultiOrUpdateWithNullExcludeUpdateProperty(List<SysUserPerm> list, Collection<String> excludeProperties);
    ...
    */
    ...
    public int multiInsert(List<SysUserPerm> list);
    public int multiInsertOrIgnore(List<SysUserPerm> list);
    public int multiInsertOrUpdate(List<SysUserPerm> list);
    public int multiInsertOrUpdateWithNull(List<SysUserPerm> list);


    /** 注意，pk必须设置值，否则报错，其它字段值为null，则丢弃此字段的更新 */
    public int update(SysUserPerm sysUserPerm);
    /** 注意，pk必须设置值，否则报错，其它字段值为null，则mysql实际也会将该字段设置为null */
    public int updateWithNull(SysUserPerm sysUserPerm);
    /** 注意，pk必须设置值，否则报错，set column=column+? where id=? */
    public int updateWithInc(SysUserPerm sysUserPerm);
    /** 注意，pk必须设置值，否则报错，乐观锁，表必须有version字段, 否则报错 */
    public int updateWithVersion(SysUserPerm sysUserPerm);

    /** 注意，自定义更新条件，字段值为null，则丢弃此字段的更新 */
    public int updateWhere(SysUserPerm sysUserPerm);
    /** 注意，自定义更新条件，字段值为null，则mysql实际也会将该字段设置为null */
    public int updateWhereWithNull(SysUserPerm sysUserPerm);
    ...
    */
    ...
    public int multiUpdate(List<SysUserPerm> list);
    /** 注意，自定义更新条件，字段值为null，则丢弃此字段的更新，mysql连接必须开启 allowMultiQueries=true */
    public int multiUpdateWhere(List<SysUserPerm> list);
    /** 注意，自定义更新条件，字段值为null，则mysql实际也会将该字段设置为null ，mysql连接必须开启 allowMultiQueries=true */
    public int multiUpdateWhereWithNull(SysUserPerm sysUserPerm);


    public int deleteByPk(Integer userId, Integer permId);
    public int delete(SysUserPerm sysUserPerm);
