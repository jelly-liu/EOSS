...
//					console.log(responseData[i].id);
	...
	$('table.eossDataTable').find('a[type=delete]').click(function(){
		...
		top.$.messager.confirm('提示','确定要执行该操作吗？', function(r){
			...
			$.ajax({
				...
				url: EossGlobal.basePath + '/permission/deletePermission.ac',
				success: function(rs){
					if(rs == 'y'){
	...
	$('table.eossDataTable').find('a[type=update]').click(function(){
		window.location.href = EossGlobal.basePath + '/permission/updatePermissionPrepare.ac?id=' + $(this).attr('value');
	});
